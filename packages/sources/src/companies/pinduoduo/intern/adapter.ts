...
function createPinduoduoChannelAdapter(
  ...
  const entryUrl = `https://careers.pddglobalhr.com/campus/${options.route}`;
  return {
    metadata: {
      key: options.key,
      version: '1.0.0',
    ...
    async *discover(context): AsyncIterable<DiscoveryEvent> {
      ...
      for (;;) {
        ...
        for (const raw of parsed.result.list) {
          ...
          yield {
            type: 'job',
            job: { externalJobId: raw.id, sourceUrl: jobUrl(entryUrl, raw.id), raw },
          };
        }
      ...
      return Promise.resolve().then(() => {
        ...
        return {
          job: parseNormalizedJob({
            ...
            detailUrl: jobUrl(entryUrl, job.id),
            applyUrl: jobUrl(entryUrl, job.id),
            publishedAt:
...
export const createPinduoduoAdapter = (): JobSourceAdapter<PinduoduoConfig, never> =>
  createPinduoduoChannelAdapter({
    ...
    route: 'intern',
    endpoint: 'https://careers.pddglobalhr.com/api/careers/api/recruit/position/train/list',
    category: 'internship',
    employmentType: '实习',
  });

export const createPinduoduoCampusAdapter = (): JobSourceAdapter<PinduoduoConfig, never> =>
  createPinduoduoChannelAdapter({
    key: 'pinduoduo.campus',
    route: 'grad',
