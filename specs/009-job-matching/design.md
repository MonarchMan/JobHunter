...
介绍过滤只在空行后的编号段落以明确职责动作或任职资格信号开头时恢复；仅空行、技术栈介绍、福利清单不触发。显式职责/要求标题始终恢复。v3.1 使用独立引擎标识与规则 ID，传递边界策略到资格、技能、实践三个消费者；v3 不启用新策略，保留旧结果重放。改动不扩展届别或技能政策。通过合成边界测试及同一冻结真实开发集复跑验证，不修改真实运行库。
