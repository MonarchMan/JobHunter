...
系统设置 `sources.activeChannel` 保存唯一活动渠道类型，默认值为 `intern`。该设置是同步范围的全局上限：公司、逻辑渠道和物理来源的既有开关仍可进一步缩小范围，但不能让非活动渠道参与同步。设置更新会同步投影到逻辑渠道和来源定时计划，并取消旧渠道尚未执行的 `source.sync` 任务；Worker 在读取物理来源时继续使用渠道有效开关作为最后一道边界。
...
Web 本地装配在查询来源前执行幂等 catalog seed。对既有公司、渠道和物理来源只刷新名称、官方入口、adapter 和支持说明等受 catalog 管理的元数据，不覆盖用户开关、来源配置、同步策略、健康、游标、同步历史和任务；不存在的记录按稳定 UUID 插入。对账完成后重新应用 `sources.activeChannel`，确保未来新增公司仍只有当前选中的渠道有效。CLI 初始化沿用同一 seed，不再要求用户删除或重建数据库。
