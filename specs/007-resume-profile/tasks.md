...
- [ ] **RES-T009** 定义经历润色 Agent、最小输入、结构化输出及事实保持校验。（RES-011,012, RES-Q05）
- [ ] **RES-T010** 实现润色建议仓储、后台任务 Handler、Web 提交与状态查询用例。（RES-011..014, RES-Q04,05）
- [ ] **RES-T011** 在在线简历加入章节选择、生成状态、建议预览与显式应用交互。（RES-011,013,014）
- [ ] **RES-T012** 添加 Agent、应用、数据库、Web 与浏览器回归测试。（RES-011..014, RES-Q04,05）
