...
Worker 由装配层注入数据库锁忙判定，应用层不依赖 SQLite 驱动。生产 Worker 的同步锁等待缩短为 100ms，领取/调度按既有空轮询间隔异步退避；心跳锁忙继续检查，提交锁忙停止本地执行并交由持久化租约恢复。周期维护接收引擎生命周期 AbortSignal，停止时立即取消，独立循环与普通调度不互相等待。

任务表新增 source_run_id、source_id、retry_root_task_id 作为不可变 payload 与重试关系的派生投影。迁移一次性回填，插入触发器统一维护所有入队入口；状态更新不重算投影。批次查询使用组合索引，普通任务使用一次批量查询关联来源与运行，返回 WebTask 安全字段。原始 payload 仍是执行输入，不新增领域数据所有者。
...
诊断仓储先将详情任务收窄为 ID、状态、时间、重试根和两个批次键，禁止把可能包含大体积来源原文的完整 payload 带入窗口排序；批次键在该物化步骤中只解析一次。筛选后的任务投影同样物化为单次查询内的 CTE，并在同一结果集中返回窗口总数和当前页。空页使用仅含总数的哨兵行，使应用层仍能计算有效页并按既有逻辑回落；正常请求不再为 `COUNT` 与分页重复执行详情重试链和 JSON 字段提取。
