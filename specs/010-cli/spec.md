...
- **CLI-004**：`jh job list|show|export` 必须支持关键词、公司、状态、地点、职位族、最低分和稳定分页。
- **CLI-005**：`jh resume import`、`jh profile show|history|set|lock|unlock` 必须覆盖导入、画像查看和人工维护。
- **CLI-006**：`jh match run|list|show` 必须支持当前画像匹配、任务状态、分项证据和建议。
...
- **CLI-011**：同步、匹配等耗时命令默认只入队并返回 task ID；显式 `--wait` 才轮询结果且可取消等待而不取消任务。
