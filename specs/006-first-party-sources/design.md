...
腾讯实习详情存在两种内容结构：普通职位使用 `desc/request`，青云计划使用 `topicDetail/topicRequirement`。适配器在来源边界同时校验两组可选字段，并按普通字段优先、青云字段兜底归一化；至少职责和要求各有一项有效内容，否则返回包含字段路径的 `parse_changed` 诊断。
