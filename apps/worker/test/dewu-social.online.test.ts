...
describe.skipIf(!online || !selected.has('dewu-social'))('Dewu social browser smoke', () => {
  it('validates the first, middle, and last official JSON pages', async () => {
    ...
    const context: DiscoverContext<ScriptedConfig> = {
      ...
      sourceId: parseId('018f0000-0000-7000-8000-000000000229', 'JobSource'),
      requestId: `dewu-social-${String(Date.now())}`,
      config,
      cursor: null,
    ...
    const adapter = createDewuSocialAdapter();
