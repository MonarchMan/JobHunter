...
- 日常平台获取复用 PlatformBrowser，同时用于职位页与来源页。职位页选定平台后原地获取一批并在终态刷新本地列表；全部平台提示先选择，不默认采集 BOSS。主要按钮说明授权最小上下文与创建 Worker 专用页，共享默认 Chrome 配置登录态、不接管已有页。取消标签页选择及 ID 输入，高级设置仅保留调试路径。初始化提示指向专用页，不暗示原页查询条件自动继承。沿用原任务进度、幂等提交和冻结恢复，不变更视觉令牌。依据 ADR-0035／0036。

- 平台工作区沿用共享 PlatformBrowser 的内联状态区：当前任务显示真实阶段、候选／已处理／已入库／排除数量；总量未知不显示虚假百分比。失败和取消保留本批统计及职位入口，脱敏类别、业务码、原因与恢复指引分开显示，不将所有异常描述成登录失败。断线由本地事件同步为不可用，刷新不访问官网；旧成功批次明确标为历史。依据 028 规格 PLT-004／006／014／018，不改变视觉令牌。

- 猎聘沿用 PlatformBrowser，入口为 provider=liepin；明确仅学生首页综合／最新推荐，连接后正常切换一次排序以初始化，不刷新。后续“读取下一批职位”由 HTTP 获取，整批补齐后在统一职位页查看；搜索和社招首页不宣称支持。轮询、失败恢复、禁用态与其他平台一致，不新增视觉令牌。依据 028 规格 PLT-004／007／018。

- 平台与官网统一使用“职位”页，来源类型默认官网，切换平台默认全部招聘类别，并可筛选具体平台；控件复用 authored SelectField，分页沿用统一职位表格。来源页保留连接和显式获取按钮，不再展示摘要职位卡片或点击保存；批次后台自动补齐详情并逐条入库，失败／取消保留已提交结果，显示稳定的职位页入口及实际入库数量（旧任务无计数则不宣称成功保存）。读取频率、保留策略不变，列表刷新不采集。依据 ADR-0030、PLT-018。

- 前程无忧复用 PlatformBrowser 表单、状态和任务轮询，URL 使用 provider=51job；按钮命名“读取官网批次”，明确先在官网搜索／翻页、活动会话只读监听及 JSON 正文复用，不伪装自动翻页。依据 ADR-0029 与 028 规格，其他平台交互保持不变。
...
- 平台推荐浏览按 028 规格使用“下一批”顺序游标，不展示虚构总页数；页面轮询只查询本地任务，刷新不重复采集。连接表单使用应用字段校验，失败保留输入并聚焦无效字段。
- BOSS 与智联共用平台浏览表单，通过 URL 链接切换 provider；切换卸载旧面板并取消其本地轮询，不断开后台连接。智联根据所选官网页面固定校园推荐或主站搜索协议，提示授权后切换校园分类或正常搜索并打开详情。主站支持范围明确为关键词／城市／经验与默认排序，不提供未实现筛选控件；查询只留连接内存，不进入站内 URL，改动需重新连接。智联统一使用“读取下一批职位”，不把搜索结果称为推荐。不展示未经证实的登录失败；隐藏页面暂停轮询，恢复时仅读取本地状态。依据 028 规格及 ADR-0028。
- 平台职位详情按 PLT-017 在客户端可见展示后一次性 POST 记录交互；预取、隐藏页和后台轮询不计。失败在详情保留内联说明及显式重试，不伪造保存成功。清理状态只读真实配置，读取失败显示未知，不提供隐式启用。
...
| Start project drill        | 建立会话 / 生成下一题  | 流式展示真实生成阶段，按钮稳定且可取消 | 留在项目档案         | 展示已校验问题     | 保留会话并直接重试   | 回到当前问题推进点           | 020        |
| Resume drill session       | 选择已暂停或已完成会话 | 禁用重复提交                           | 留在项目档案         | 会话恢复为进行中   | 待处理任务结束后重试 | 聚焦恢复会话的当前推进点     | 020        |
...
- Recruitment platform workspace: 四平台导航复用来源导航所有者，以 URL 和 aria-current 同步可见选中状态，窄屏两列而非挤压长名称。连接状态使用既有语义色与文字，任务进度和长诊断单独分区且可换行；采集失败不自动展开连接表单，真实采集进度不提示浏览器授权。职位清除普通筛选及空状态恢复保留 provider；任务名称与筛选共用四平台完整映射。依据 PLT-014、PLT-016、PLT-018。
...
- Auto-save/draft recovery: 设置通过“保存设置”显式提交，复选框修改仅更新本地草稿；在线简历显式整份保存。失败与版本冲突均保留客户端输入，不自动覆盖新版本。自动评分勾选表示启用，取消表示停用，保存后才影响未来新修订的任务创建，见 WEB-016。
...
- Project drill pending ownership: 项目问题在当前 Web 请求中同步生成，界面通过 SSE 流展示由服务端真实执行点驱动的准备上下文、生成、校验和保存阶段，不显示虚假百分比或未校验正文；等待态稳定且允许取消。成功后一次展示通过结构化和证据校验的问题，失败保留会话并允许直接重试。回答分析和 Markdown 投影仍只轮询真实 Task 状态，页面隐藏时不刷新。暂停或完成会话前必须先等待或取消当前操作，迟到 Agent 结果以会话修订号拒绝。
...
- Project drill session recovery: 项目档案详情按最后修改时间倒序展示全部会话，并以“会话 - YYYY-MM-DD HH:mm:ss”命名；默认进入 active 会话，否则进入最近修改会话。已完成会话可显式继续，恢复时原 active 会话自动暂停；若其任务仍在处理则保留状态并提示稍后重试。
...
- Resume studio Safari boundary: 编辑画布通过 CSP 禁止文档脚本与外部资源，同时允许可信父页事件在 Safari 执行；预览保持禁脚本沙箱。新增、删除、输入失焦保存及重载恢复必须覆盖 WebKit 和 Chromium，见 ADR-0022。

- Resume block actions: 每个文本块（而非章节或整段经历）在悬停／键盘聚焦时显示珊瑚色虚线框与右下方 24px 粉底浮动 +1／+2／+3、删除按钮，不挤占空间。新增插入当前块后，删除仅移除当前块，未添加章节不在正文占位，左侧管理章节增删；有内容的章节删除需应用内确认；新增后聚焦新块。列表以回车续写、行首退格取消圆点，Ctrl／⌘+Shift+8 操作当前段落。预览与导出没有操作栏。

- Resume content blocks: 工作／项目描述在一个编辑区内保留普通段落与列表；回车续写职责，行首退格取消圆点，中文输入法确认不触发快捷键。普通章节可添加 1–3 栏内容块组，每栏允许多段文字和列表。失焦仅保存快照，不重载画布；粘贴只接收纯文本。依据 RTE-003。
