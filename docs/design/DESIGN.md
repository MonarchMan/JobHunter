...
首页“面试充电站”沿用工作台面板间距，以紧凑白色链接卡片展示网站名、方向标签与一句话说明；宽屏四列、中屏两列、窄屏单列。网站名前展示本地保存的官方 Logo／站点图标，固定 32px 并保持原比例与原色，不请求第三方图片；外链箭头和新标签提示说明导航行为，hover 改变边框与标题颜色，焦点保持清晰，不添加独立品牌强调。
...
项目拷打使用“准备档案工作台”变体，不采用聊天气泡。主栏一次只突出当前问题或唯一下一步，3px 珊瑚色决策光标标记该推进点；右栏以无总分的十维覆盖台账记录证据状态，完成记录以时间线保留问题、原始回答修订和可追溯推导。档案详情在快照下提供紧凑的会话索引，会话按最后修改时间倒序并以秒级时间命名，当前查看会话用浅靛蓝底和珊瑚色光标标记；已完成会话可显式继续修改。项目列表使用连续档案行和共享表面，避免将每个状态拆成统计卡。
...
简历制作使用“投递排版台”变体：进入后隐藏后台全局导航，顶部保留返回、模板身份、持久保存状态、模板预览、导出和当前章节的字号/字距/行高工具；桌面排版控件组与 A4 画布水平中心对齐，章节说明和结构动作分别停靠两侧。左侧提供紧凑章节索引和章节增删图标，主体是一张真实比例 A4 纸面，投递字段可在对应文字块中直接原位编辑。当前章节只以靛蓝细轮廓和 3px 珊瑚色编辑光标标记，导出时完全移除。模板预览使用同一模板的非交互 A4 快照，独立于在线简历预览。输出模板拥有独立、封闭的排版色板：“简洁单页”使用深墨顶轨、图标化章节、浅蓝信息卡和紧凑时间轴；其姓名与灰蓝色求职方向在同一基线形成主次标题，专业技能使用完整句子无序列表。“标准单页”使用标准单列、细分隔与克制靛蓝。模板色不得泄漏到后台控件，后台令牌也不得改写模板的投递版式。轻量返回与标题按钮保持透明背景，悬停只改变文字颜色，不发生位移。

投递画布的项目／工作描述采用整体编辑边框，内部段落与列表自然流动；普通文本块支持全宽单栏或并排双／三栏，每栏高度随多段文字增长。每个文本块在悬停或键盘聚焦时显示珊瑚色虚线轮廓，右下方悬浮高 24px 的粉底 +1／+2／+3 与删除图标（action-soft／danger-soft 背景、danger 文字），不占排版空间；图标具有可访问名称。预览和导出不显示这些控件。
