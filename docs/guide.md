...
在“职位”页把来源类型切换为“招聘平台”，再选择 BOSS、智联、前程无忧或猎聘。在日常使用的 Chrome 登录平台并启用远程调试；确认借用最小登录上下文后点击“获取职位”。Worker 会在同一默认配置中新建专用标签页，共享登录态，不接管已有页面；后续获取复用该页和连接。每次只取一批并补齐详情入库，进度和结果留在职位页。不需要选择标签页或填写其 ID；Chrome 出现授权提示时仍需允许。原页面的搜索条件不会自动复制，初始化和筛选操作应在新建的专用页完成。断开只关闭 Worker 创建的页；意外断线遗留的专用页可手动关闭。

智联日常入口新建主站搜索页，首次连接需在该页搜索并打开一条详情；校园协议仍可通过 CLI 显式目标调试入口使用。猎聘目前仅学生首页，若新页没有产生推荐请求，需正常切换一次排序；前程无忧需要在专用页正常搜索／翻页后读取该批。不会自动刷新官网或抓取所有页；职位页的筛选仅筛选本地库，不修改官网查询。修改查询时先断开、重新获取，再在新专用页设置条件。
