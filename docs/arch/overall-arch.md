...
- 按计划执行来源同步。
- 执行失败重试、语义增强和匹配重算。
- 执行面试回答摘要、Markdown 准备文档投影和外部网友面经研究任务；不执行新的项目问题生成请求。
- 恢复进程异常退出时遗留的超时任务。
- 维护来源健康状态和同步游标。
...
Web 启动器负责启动 Web 进程和独立 Worker 子进程，并在任一子进程退出时联动关闭另一个。Web 管理台复用应用层与仓储端口，提供查询、筛选、配置、项目拷打、个人面经导入和网友面经研究审核。项目拷打的单题问题生成是明确例外：Web 在当前 HTTP 请求内调用模型，并通过 SSE 流发送不含问题正文的真实执行阶段，但模型调用不进入数据库事务；回答摘要、投影、外部 Agent 和其他耗时任务仍只入队。显式 Markdown 上传只在边界完成有上限的清洗、分块和文件版本登记，不产生专用后台预处理任务。
...
8. 网友面经以 `ExperienceResearchRequest` 保存研究意图，Prompt、JSON Schema 和 Bundle 都是通用文件版本；预览和执行读取请求冻结的精确 Prompt/Schema 版本，人工导包、`codex-local@v1` 与只兼容当前 `community-research-prompt@v4` 的 `browser-assisted-codex@v2` 汇入同一 Bundle Importer，只有人工接受后才出现在网友面经。问题周边证据摘录不进入 Bundle 或数据库，用户通过原始来源链接核对上下文；旧 Prompt 不得静默升级。
...
- 简历内容和密钥不会出现在普通日志中。
- 项目拷打问题可追溯到允许的输入快照，且不能读取源码、生成代答或把网友陈述写成用户事实。
- 外部 Agent 只能返回待审核 Artifact，不能直接修改业务存储。
