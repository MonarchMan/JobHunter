...
Web 管理台复用应用层与仓储端口，主要提供查询、筛选、配置和手动操作。耗时采集或模型任务只入队，不在 HTTP 请求中执行。
...
| `JobSource`        | 官网入口、适配器类型、配置、同步策略和健康状态 |
...
公开 JSON API
  > 页面内嵌结构化数据
  > 静态 HTML 解析
  > 受控浏览器执行官网公开脚本并读取渲染结果
```

受控浏览器可以正常执行官网下发的公开前端脚本，包括由该脚本在当前匿名会话中生成请求所需参数；不得逆向伪造风控参数、绕过验证码、复用登录 Cookie 或隐藏自动化身份以规避访问控制。浏览器实现必须封装在来源基础设施内，仍遵循相同适配器契约，且浏览器细节不能泄露到应用层。
