...
当前默认规则为 v3.1：纯函数引擎从冻结职位招聘类别选择社招、校招、实习权重，资格门槛与相关性得分分离。条件树保留局部修饰、上下限与三值逻辑；技能按任选组和正向使用证据计算，实践按技能、职责和贡献计算，不按长度或数字数量加分。画像可选 matchingConstraints 保存用户确认的资格事实，分项 JSON 保存证据状态。v3.1 仅增加介绍后无标题编号职责/要求的保守恢复策略。应用入口激活独立 v3.1 规则 ID；v1/v2/v3 定义、计算路径与历史记录保留，不全库重算。未知维度不补分、不重新分配权重，“经验不限”只豁免年限门槛。具体政策与验收以 [009 规格](../../specs/009-job-matching/spec.md) 为准，未改变依赖方向或数据所有权。
