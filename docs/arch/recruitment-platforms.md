...
平台收尾沿用现有任务数据所有权：阶段、批次数量及白名单错误保存在 tasks.result_json，职位提交事务原子更新已入库计数，失败／取消保留部分结果；不新增职位表或运行进程。CDP 本地断线订阅按平台、代次及会话身份更新状态，不以平台请求探活。观察命令通过 provider 独立选择任务域和代次，每轮只取一批并验证入库，移除额外详情和固定 90 秒整批限制；末批是正常结束，某个平台失败不停止其他平台。跨平台共用浏览器 Socket 未在本轮实现，实验节流不变。
