...
简历制作使用“投递排版台”变体：进入后隐藏后台全局导航，顶部只保留返回、模板身份、持久保存状态和导出；左侧章节编辑栏保持产品令牌与表单行为，主体是一张真实比例的 A4 纸面。当前章节只以靛蓝细轮廓和 3px 珊瑚色编辑光标标记，导出时完全移除。输出模板拥有独立、封闭的排版色板：“技术蓝图”使用深墨顶轨、清晰蓝色章节线和时间轴；“简洁单栏”使用高密度单列、细分隔与克制靛蓝。模板色不得泄漏到后台控件，后台令牌也不得改写模板的投递版式。
