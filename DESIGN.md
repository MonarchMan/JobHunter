...
来源公司卡使用紧凑“运行台账”变体：标题与控制区只保留完成识别和操作所需的间距，不用固定内容高度制造整齐假象；公司总览的四项指标共享一个分隔式数据条，宽卡单行、窄卡两列，禁止再次拆成四张大号统计卡。右上角使用无圆点的纯文字标签显示系统当前同步招聘渠道；健康状态内侧色轨保留为唯一显著结构信号，其余表面保持白色和细分隔线。
...
简历制作使用“投递排版台”变体：进入后隐藏后台全局导航，顶部保留返回、模板身份、持久保存状态、模板预览、导出和当前章节的字号/字距/行高工具；桌面排版控件组与 A4 画布水平中心对齐，章节说明和结构动作分别停靠两侧。左侧只保留紧凑章节索引，主体是一张真实比例 A4 纸面，投递字段可在对应文字块中直接原位编辑。当前章节只以靛蓝细轮廓和 3px 珊瑚色编辑光标标记，导出时完全移除。模板预览使用同一模板的非交互 A4 快照，独立于在线简历预览。输出模板拥有独立、封闭的排版色板：“简洁单页”使用深墨顶轨、图标化章节、浅蓝信息卡和紧凑时间轴；其姓名与灰蓝色求职方向在同一基线形成主次标题，专业技能使用完整句子无序列表。“标准单页”使用标准单列、细分隔与克制靛蓝。模板色不得泄漏到后台控件，后台令牌也不得改写模板的投递版式。轻量返回与标题按钮保持透明背景，悬停只改变文字颜色，不发生位移。

### Iconography

使用一致的 1.75px 线性 SVG 图标，常用尺寸 18px 和 20px。非通用图标必须保留文字或可访问名称，禁止混用 emoji 作为产品图标。

JobHunter 主品牌标记使用 `apps/web/public/assets/brand/jobhunter-logo.png`：H 表达机会之门，J 的笔画与负空间表达迈步取得 offer 的求职者，珊瑚色卡片是唯一强调。导航中与“JobHunter”文字并列使用约 30px 标记，图片保持空替代文本；工作台 Hero 可将其放大为品牌锚点，但不得作为无意义的重复卡片或扩散到数据密集页面。
